...
class ViewKCircleSpread @JvmOverloads constructor(
    ...
    override fun requireStart() {
        _isStop = false
        _spreadCircles.add(_centerRadius to 255f)
        postInvalidate()
    }
    ...
    override fun initAttrs(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        _centerRadius =
            typedArray.getDimension(R.styleable.ViewKCircleSpread_viewKCircleSpread_centerRadius, _centerRadius)
        _spreadColor =
        ...
        _spreadIsRipple =
            typedArray.getBoolean(R.styleable.ViewKCircleSpread_viewKCircleSpread_spreadIsRipple, _spreadIsRipple)
        _circlesDistance =
            typedArray.getDimension(R.styleable.ViewKCircleSpread_viewKCircleSpread_circlesDistance, _circlesDistance)
        _animTime =
            typedArray.getInteger(R.styleable.ViewKCircleSpread_viewKCircleSpread_animTime, _animTime)
        typedArray.recycle()
    ...
    private fun changeRadiusAndAlpha() {
        ...
        if (_spreadCircles[_spreadCircles.size - 1].first - _centerRadius >= _circlesDistance && _spreadCircles.size < _realSpreadCount && !_isStop) {
            _spreadCircles.addLast(_centerRadius to 255f)
        }
    ...
    private fun drawCircleCenter(canvas: Canvas) {
        ...
        } ?: canvas.drawCircle(centerX, centerY, _centerRadius, _centerPaint)
    }
