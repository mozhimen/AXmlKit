...
class ViewKSteps @JvmOverloads constructor(
    ...
    override fun initAttrs(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        _margin =
            typedArray.getDimension(R.styleable.ViewKSteps_viewKSteps_margin, _margin)
        _titleMarginTop =
            typedArray.getDimension(R.styleable.ViewKSteps_viewKSteps_titleMarginTop, _titleMarginTop)
        _doneLineColor =
            typedArray.getColor(R.styleable.ViewKSteps_viewKSteps_doneLineColor, _doneLineColor)
        _undoLineColor =
            typedArray.getColor(R.styleable.ViewKSteps_viewKSteps_undoLineColor, _undoLineColor)

        _doneNumberColor =
            typedArray.getColor(R.styleable.ViewKSteps_viewKSteps_doneNumberColor, _doneNumberColor)
        _undoNumberColor =
            typedArray.getColor(R.styleable.ViewKSteps_viewKSteps_undoNumberColor, _undoNumberColor)

        _doneTitleColor =
            typedArray.getColor(R.styleable.ViewKSteps_viewKSteps_doneTitleColor, _doneTitleColor)
        _undoTitleColor =
            typedArray.getColor(R.styleable.ViewKSteps_viewKSteps_undoTitleColor, _undoTitleColor)

        _circleRadius =
            typedArray.getDimension(R.styleable.ViewKSteps_viewKSteps_circleRadius, _circleRadius)
        _lineWidth =
            typedArray.getDimension(R.styleable.ViewKSteps_viewKSteps_lineWidth, _lineWidth)
        _numberTextSize =
            typedArray.getDimension(R.styleable.ViewKSteps_viewKSteps_numberTextSize, _numberTextSize)
        _titleTextSize =
            typedArray.getDimension(R.styleable.ViewKSteps_viewKSteps_titleTextSize, _titleTextSize)
        typedArray.recycle()
    }
    ...
    override fun initPaint() {
        ...
        _numberPaint.textSize = _numberTextSize
        ...
        _titlePaint.textSize = _titleTextSize
        _titlePaint.typeface = Typeface.DEFAULT_BOLD // 采用默认的宽度
        _titlePaint.color = _undoTitleColor
    }

    override fun initData() {
        _realCenterY = _circleRadius + paddingTop + _margin
        _leftX = (this.left + paddingLeft + _margin + _circleRadius)
        _leftY = _realCenterY - _lineWidth / 2f
        _rightX = (this.right - paddingRight - _margin - _circleRadius)
