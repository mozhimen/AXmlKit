...
class ImageKMask @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private companion object {
        ...
        private val MASK_COLOR = UtilKRes.gainColor(com.mozhimen.uik.R.color.cok_blue_0177fd)
    ...
    private fun updateLocSingleFinger(ax: Float, ay: Float): Pair<Float, Float> {
        ...
        val distance = UtilKMathPoint.getDistance(ax, ay, _lastAnchorTwoFinger!!.first, _lastAnchorTwoFinger!!.second)
    ...
    private fun updateLocTwoFinger(ax: Float, ay: Float, bx: Float, by: Float): Pair<Float, Float> {
        val point = UtilKMathPoint.getCenter(ax, ay, bx, by)
        if (_lastAnchorTwoFinger == null) return point
        val distance = UtilKMathPoint.getDistance(point.first, point.second, _lastAnchorTwoFinger!!.first, _lastAnchorTwoFinger!!.second)
    ...
    private fun updateScale(ax: Float, ay: Float, bx: Float, by: Float): Float {
        val fingerDistance = UtilKMathPoint.getDistance(ax, ay, bx, by)
        ...
        val angle: Float = if (ax != bx) {
            ...
            if (rightY == _lastAnchorTwoFinger!!.second) {
                ...
                val hypotenuse = UtilKMathPoint.getDistance(rightX, rightY, _lastAnchorTwoFinger!!.first, _lastAnchorTwoFinger!!.second)
                val tempAngle = UtilKMathTriangle.getAngleSin(opposite, hypotenuse)
                if (rightY < _lastAnchorTwoFinger!!.second) {
                    tempAngle
                } else {
                    180f - tempAngle
    ...
    private class MaskRect {
        ...
        fun isTapInArea(e: MotionEvent): Boolean =
            UtilKMathArea.isInside_android(e.x, e.y, leftX, topY, rightX, bottomY)
    }
}