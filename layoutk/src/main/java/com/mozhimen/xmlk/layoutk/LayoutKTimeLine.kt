...
import com.mozhimen.xmlk.basic.commons.IAttrsParser
...
class LayoutKTimeLine @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : BaseLayoutKLinear(context, attrs, defStyleAttr) {
    ...
    override fun initAttrs(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        _pointIcon = (UtilKRes.gainDrawable(_attrs.pointResId) as BitmapDrawable).bitmap
    }
    ...
    fun setPointIconResId(resId: Int) {
        _pointIcon = (UtilKRes.gainDrawable(resId) as BitmapDrawable).bitmap
        invalidate()
    }
    ...
    private object AttrsParser : IAttrsParser<Attrs> {
        ...
        val LINE_WIDTH: Int = 2f.dp2pxI()//line property
        val LINE_COLOR: Int = UtilKRes.gainColor(R.color.blue_normal)
        const val LINE_GRAVITY: Int = GRAVITY_LEFT//line gravity(默认垂直的左边)
        val POINT_SIZE: Int = 0f.dp2pxI()//point property
        val POINT_COLOR: Int = LINE_COLOR
        val POINT_RES_ID: Int = R.drawable.viewk_qr_scan_success

        override fun parseAttrs(context: Context, attrs: AttributeSet?): Attrs {
