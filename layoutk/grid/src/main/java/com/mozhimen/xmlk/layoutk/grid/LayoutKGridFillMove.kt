...
import com.mozhimen.xmlk.layoutk.untouch.LayoutKUnTouchGrid
...
class LayoutKGridFillMove @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private val _cellDefaultColor = "#D8D5D7".strColor2intColor()    //默认格子颜色
    private val _cellHoverColor = "#A6D8A9".strColor2intColor()    //滑动时进入格子的暂时颜色
    private val _cellSelectColor = "#4FD855".strColor2intColor()    //选中的颜色
    private val _dashLineWidth = 3f.dp2px()

    private lateinit var _cells: Array<Array<View>>    //二维格子
    private var _cellLast: View? = null    //上一个  view

    private val _areaSampleCell: MArea = MArea(30f.dp2pxI(), 30f.dp2pxI(), 30f.dp2pxI(), 50f.dp2pxI())    //格子大小
    ...
    private val _layoutKGridFill by lazy {
        LayoutKUnTouchGrid(context).apply {
    ...
    private fun initCells() {
        _cells = Array(_gridRowCount, init = {
            ...
            Array(_gridColumnCount, init = {
                View(context).apply {
                    val layoutParams = LayoutParams(30f.dp2pxI(), 30f.dp2pxI())
                    this.layoutParams = layoutParams
